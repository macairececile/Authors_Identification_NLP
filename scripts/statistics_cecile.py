...
def collocations(file):
    ...
    return finder_bigram.nbest(bigram_measures.pmi, 10)


def counting_vocabulary(file):
    """Counting vocabulary of a text"""
    text = read_file(file)
    tokens_text = word_tokenize(text)
    return len(tokens_text)


def lexical_richness(file):
    """Counting lexical richness"""
    text = read_file(file)
    tokens_text = word_tokenize(text)
    vocabulary = len(set(tokens_text))
    return (vocabulary / len(tokens_text)) * 100


def frequency_distribution(file):
    """20 most used vocabulary item in the text"""
    text = read_file(file)
    tokens_text = tokens(text)
    frequency = nltk.FreqDist(tokens_text)
